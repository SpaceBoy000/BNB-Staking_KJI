...
export function prettyVestingPeriod2(depositTime: number, lockTime: number) {
  if (depositTime == 0) {
    return "0";
  }

  console.log("deposit time: ", depositTime.toString());
  console.log("Lock time: ", lockTime.toString());
  console.log("Current time: ", (new Date()).getTime() / 1000);

  const seconds = (depositTime - (new Date()).getTime() / 1000 + lockTime * 1.0);
  if (seconds < 0) {
    return "Fully Vested";
  }
